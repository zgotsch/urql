...
function transportDataToJS(data) {
  ...
  return `(window[Symbol.for("${key}")] ??= []).push("${Buffer.from(JSON.stringify(data), 'utf-8').toString('base64')}")`;
}
...
function useUrqlValue(operationKey) {
  ...
  if (typeof window == 'undefined') {
    ...
    if (window[urqlTransportSymbol]) {
      stores = window[urqlTransportSymbol].map(s => JSON.parse(decodeBase64(s)));
    } else {
      stores = [];
    }
